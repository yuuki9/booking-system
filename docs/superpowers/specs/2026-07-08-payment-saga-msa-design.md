...
5. 패키지 이동 시 기존 패키지명 `com.lab.reservation`은 유지한다. 신규 코드만 `com.lab.contracts`, `com.lab.payment`를 사용한다.
...
| 원칙 | 적용 |
|------|------|
| Database per service | `booking_system` DB는 reservation-service만, `payment_db`는 payment-service만 접근 |
...
Client / k6
 ...
 Nginx (scale3 프로필, 기존 유지)
    ...
    ▼
┌─────────────────────────────┐          ┌─────────────────────────────┐
│ reservation-service (8080)  │          │ payment-service (8081)      │
│                             │          │                             │
...
├── docker-compose.yml                # payment-service, payment_db init 추가
├── docker-compose.infra.yml          # payment_db init 추가
├── contracts/
│   ├── build.gradle.kts
│   └── src/main/kotlin/com/lab/contracts/
...
│       └── main/kotlin/com/lab/reservation/
│           ├── ... (기존 구조 유지)
...
토픽 생성: 로컬(`!aws` 프로파일)에서는 각 서비스의 `KafkaConfig`가 `NewTopic` bean으로 생성(파티션 3, replica 1 — 기존 `reservation.confirmed`와 동일). `reservation.pending`·`reservation.confirmed`는 reservation-service가, `payment.result`는 payment-service가 생성한다.

### 4.2 이벤트 DTO (아래 코드 그대로 구현)

```kotlin
// contracts/src/main/kotlin/com/lab/contracts/ReservationPendingEvent.kt
package com.lab.contracts
...
```kotlin
// contracts/src/main/kotlin/com/lab/contracts/PaymentResultEvent.kt
package com.lab.contracts
...
```kotlin
// contracts/src/main/kotlin/com/lab/contracts/ReservationConfirmedEvent.kt
package com.lab.contracts

import java.time.Instant
import java.util.UUID

// 기존 com.lab.reservation.kafka.ReservationConfirmedEvent를 필드 변경 없이 이동
...
- 각 서비스 `application.yml`에서 `spring.json.trusted.packages: com.lab.contracts`로 변경하고, 기존의 `spring.json.value.default.type` 설정은 **제거**한다 (토픽별 이벤트 타입이 여러 개가 되므로 default type은 오동작 원인).
...
Payment 상태: `PENDING → APPROVED | FAILED` (payment-service 내부).

### 5.2 정상 흐름 (happy path)

```
Client          reservation-service        Kafka           payment-service
  │                    │                     │                    │
  │ POST /reservations │                     │                    │
  ├───────────────────▶│                     │                    │
  │                    │ 멱등키 확인            │                    │
...
```
payment-service: MockPG 거절 → payments FAILED(DECLINED) → payment.result(FAILED)
        │
        ▼
reservation-service (PaymentResultConsumer):
...
1. **payment-service 내부 타임아웃**: MockPG가 지연/타임아웃을 시뮬레이션하면 payment-service가 `FAILED(TIMEOUT)`으로 기록하고 `payment.result`를 발행한다. (정상 경로로 보상이 돈다.)
2. **reservation-service reaper**: `payment.result` 자체가 유실된 경우의 안전망. `@Scheduled`(10초 주기)로 `PENDING_PAYMENT`이면서 `created_at`이 `app.standard.payment.timeout-seconds`(기본 60초)보다 오래된 예약을 5.3과 동일한 보상 경로로 `CANCELLED` 처리한다.
...
| 위치 | 메커니즘 |
|------|----------|
| payment-service의 `reservation.pending` 소비 | `payments.reservation_id UNIQUE` 제약. 중복 INSERT 시 `DataIntegrityViolationException` catch → 로그 후 skip |
| reservation-service의 `payment.result` 소비 | status 가드 UPDATE (`WHERE status='PENDING_PAYMENT'`). 0행이면 skip |
...
@KafkaListener(topics = ["\${app.kafka.topic.payment-result}"], groupId = "reservation-service")
...
### 6.8 설정 추가 (`reservation-service/src/main/resources/application.yml`)
...
`AppModeProperties`에 `standard.payment` 프로퍼티 바인딩 추가. consumer 역직렬화 설정은 4.3 참조 (`trusted.packages: com.lab.contracts`, `default.type` 제거).
...
- Consumer: `@KafkaListener(topics = ["reservation.pending"], groupId = "payment-service", concurrency = "3")` → `PaymentProcessingService.process()`.
- OutboxPublisher: reservation-service의 `StandardOutboxPublisher`와 동일 패턴. 2초 폴링, 미발행분을 `PaymentResultEvent`로 매핑해 `payment.result` 토픽 발행 (key = reservationId), `published_at` 갱신.

### 7.5 `application.yml` (payment-service)
...
spring:
  ...
  kafka:
    ...
    consumer:
      ...
      properties:
        spring.json.trusted.packages: com.lab.contracts
...
### 8.4 `reservation-service/build.gradle.kts`
...
### 8.5 `payment-service/build.gradle.kts`

reservation-service와 동일 골격에서 의존성만 축소:
...
4. `payment-service` 서비스 신규 (single·scale3 양쪽에서 필요하므로 두 프로필 모두 지정):
...
(payment-service 미기동 시 테이블이 없을 수 있으므로 `|| true`로 무시 처리.)
...
- `LockStrategyIntegrationTest`, `LockStrategyResolverTest`는 reservation-service 모듈로 이동 후 **무수정 통과**해야 한다 (flag 기본 off이므로).
- 실행: `./gradlew :reservation-service:test`

### 10.2 신규 단위 테스트

| 테스트 | 검증 내용 |
|--------|-----------|
| `MockPaymentGatewayTest` (payment-service) | `fail-` prefix → Declined, `timeout-` prefix → TimedOut, rate=0이면 항상 Approved, failure-rate=1.0이면 항상 Declined |
| `StandardPaymentSagaServiceTest` (reservation-service) | APPROVED: PENDING→CONFIRMED 전이 + outbox 적재. FAILED: CANCELLED 전이 + reserved_count 감소 + Redis rollback 호출. 이미 CONFIRMED/CANCELLED인 예약에 대한 이벤트 → 아무 것도 안 함 |
| `PaymentProcessingServiceTest` (payment-service) | 정상 처리 → APPROVED + outbox. 중복 reservation_id → skip |
...
1. `reservation-service/` 디렉토리 생성, 기존 `src/` 전체를 `reservation-service/src/`로 이동 (git mv).
2. 8.1~8.4의 빌드 파일 작성 (contracts, payment-service는 아직 빈 껍데기 또는 미포함 가능 — 미포함 권장: settings에는 reservation-service만 먼저 include해도 됨).
3. Dockerfile을 8.6으로 교체, docker-compose의 build를 9.2-3 형식으로 변경.

**DoD**: `./gradlew :reservation-service:test` 전체 통과. `docker compose --profile single up -d --build` 후 기존 README의 curl 예제(예약 생성 201) 동작. `./scripts/reset-standard.sh` 동작.

### Phase 1 — contracts 모듈 + 이벤트 이동

1. `contracts` 모듈 생성, 4.2의 3개 이벤트 클래스 작성.
2. `com.lab.reservation.kafka.ReservationConfirmedEvent` 삭제, 참조를 `com.lab.contracts.ReservationConfirmedEvent`로 교체.
...
### Phase 2 — reservation-service Saga 지원 (flag off 기본)
...
### Phase 3 — payment-service 신규

1. 모듈 생성 (8.5), 스키마 (7.1), 엔티티/리포지토리, MockPaymentGateway (7.2), ProcessingService (7.3), Consumer/OutboxPublisher (7.4), application.yml (7.5), KafkaConfig (payment.result NewTopic).
2. 단위 테스트 (10.2).

**DoD**: `./gradlew :payment-service:test` 통과. infra compose + 양 서비스 로컬 기동으로 happy path 수동 확인 (PENDING_PAYMENT → CONFIRMED).

### Phase 4 — Docker Compose 통합

1. 9장 전체 반영 (init 스크립트, payment-service 서비스, PAYMENT_ENABLED, reset 스크립트).
...
9. **Flyway 분리**: payment-service의 migration은 `payment-service/src/main/resources/db/migration`에 V1부터 새로 시작한다. reservation의 V1·V2와 무관하다.
10. **aws 프로파일**: `application-aws.yml`은 reservation-service로 함께 이동만 하고 내용 수정은 이번 범위에서 제외 (MSK 토픽 pre-provision 등 AWS 반영은 별도 작업).
