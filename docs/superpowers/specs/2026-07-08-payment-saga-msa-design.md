...
- 작성일: 2026-07-08
- 갱신일: 2026-07-09
- 상태: 설계 확정 (Phase 0~6 완료)
- 갱신일: 2026-07-10
- Phase 6 구현 계획: `docs/superpowers/plans/2026-07-10-phase6-late-approval-refund.md`
...
2. **reservation reaper**: `payment.result` 자체가 유실된 경우의 안전망. `@Scheduled`(10초 주기)로 `PENDING_PAYMENT`이면서 `created_at`이 `app.standard.payment.timeout-seconds`(기본 60초)보다 오래된 예약을 5.3과 동일한 보상 경로로 `CANCELLED` 처리한다.
   ...
   - **Phase 6 (환불 보상)**: reaper가 먼저 `CANCELLED`로 만든 뒤 늦은 `APPROVED`가 오면 reservation이 `payment.refund` Outbox를 발행하고 payment가 Mock PG 환불 후 `REFUNDED`로 전이한다.
...
### Phase 6 — 늦은 APPROVED 환불 보상 ✅

1. reaper/`CANCELLED` 이후 도착한 `payment.result(APPROVED)`에서 `payment.refund` Outbox 발행.
2. payment: Mock PG `refund` + `REFUNDED` 상태 (멱등).
3. README·§5.4에서 “결제만 성공·좌석 없음”을 해결된 시나리오로 갱신.

상세 구현 스텝: `docs/superpowers/plans/2026-07-10-phase6-late-approval-refund.md`

**DoD**: 통합 테스트로 late-approve→REFUNDED 증명. happy/fail/reaper 회귀 통과.
...
- [x] 알려진 한계와 실무 확장 방향 (늦은 승인 환불 Saga, PG 트랜잭션 분리 — Phase 6·TX 분리 반영)
